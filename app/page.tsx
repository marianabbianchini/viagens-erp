...
import { readTicket } from "../lib/ticket-reader";
...
function ImportForm({ onDone }: { onDone: () => void }) {
  ...
  const [saving,setSaving]=useState(false); const [reading,setReading]=useState(false); const [error,setError]=useState(""); const [fileName,setFileName]=useState("");
  const update=(key:string,value:string)=>setForm(prev=>({...prev,[key]:value}));
  async function importFile(file?:File){if(!file)return;setReading(true);setError("");setFileName(file.name);try{const {fields}=await readTicket(file);setForm(prev=>({...prev,...Object.fromEntries(Object.entries(fields).filter(([,value])=>value))}));if(!Object.values(fields).some(Boolean))setError("Não consegui identificar os dados. Você pode preencher os campos manualmente.");}catch(err){setError(err instanceof Error?`Não foi possível ler o arquivo: ${err.message}`:"Não foi possível ler o arquivo.");}finally{setReading(false)}}
  ...
  return <div className="drawer-content"><p className="eyebrow">NOVA RESERVA</p><h2>Adicionar viagem</h2><p className="drawer-subtitle">Envie o e-ticket, confira os dados identificados e ajuste o que for necessário antes de salvar.</p><label className="upload"><span>{reading?"◌":"⇧"}</span><strong>{reading?"Lendo o e-ticket...":fileName||"Enviar e-ticket"}</strong><small>PDF, PNG ou JPG • processamento no navegador</small><input type="file" accept=".pdf,image/png,image/jpeg" disabled={reading} onChange={e=>void importFile(e.target.files?.[0])}/></label>{fileName&&!reading&&<p style={{color:"#239c65",fontSize:9,textAlign:"center",marginTop:8}}>✓ Arquivo analisado. Confira os campos abaixo.</p>}<div className="divider"><span>dados para conferência</span></div><div className="form-grid"><label>Companhia aérea<select value={form.airline} onChange={e=>update("airline",e.target.value)}><option value="">Selecione...</option>{airlines.map(a=><option key={a}>{a}</option>)}</select></label><label>Localizador<input value={form.locator} onChange={e=>update("locator",e.target.value)} placeholder="Ex.: PNR123" maxLength={8}/></label><label className="full">Nome do passageiro<input value={form.passenger} onChange={e=>update("passenger",e.target.value)} placeholder="Nome como consta no bilhete"/></label><label>Sobrenome<input value={form.surname} onChange={e=>update("surname",e.target.value)} placeholder="Para consultar a reserva"/></label><label>Status<select value={form.status} onChange={e=>update("status",e.target.value)}><option>Confirmada</option><option>Pendente</option></select></label><label>Origem<input value={form.origin} onChange={e=>update("origin",e.target.value)} placeholder="CWB" maxLength={3}/></label><label>Destino<input value={form.destination} onChange={e=>update("destination",e.target.value)} placeholder="GRU" maxLength={3}/></label><label>Data<input type="date" value={form.departureDate} onChange={e=>update("departureDate",e.target.value)}/></label><label>Horário<input type="time" value={form.departureTime} onChange={e=>update("departureTime",e.target.value)}/></label></div>{error&&<p style={{color:"#c5414c",fontSize:10}}>{error}</p>}<button className="primary-button submit" disabled={saving||reading} onClick={submit}>{saving?"Salvando...":"Confirmar e gerar bilhete"}</button></div>;
}
...
function Ticket({ trip,onDeleted }: { trip: Trip;onDeleted:()=>void }) {
  ...
  const [from,to]=trip.airports.split("•").map(value=>value.trim());
  return <div className="drawer-content"><p className="eyebrow screen-only">DETALHES DA VIAGEM</p><h2 className="screen-only">{trip.route}</h2><p className="drawer-subtitle screen-only">Reserva {trip.locator} • {trip.airline}</p><div className="ticket"><div className="ticket-accent" style={{background:trip.color}}/><div className="ticket-head"><div className="airline-wordmark" style={{color:trip.color}}><span>{trip.airline[0]}</span><strong>{trip.airline}</strong></div><div className="document-title"><strong>CONFIRMAÇÃO DE VIAGEM</strong><small>ITINERÁRIO DO PASSAGEIRO</small></div><div className="booking-code"><small>LOCALIZADOR</small><code>{trip.locator}</code></div></div><div className="ticket-route"><div><small>ORIGEM</small><strong>{from}</strong></div><span><i>✈</i></span><div><small>DESTINO</small><strong>{to}</strong></div></div><div className="flight-strip"><span>VOO OPERADO POR <strong>{trip.airline.toUpperCase()}</strong></span><span className={`status ${trip.status==="Confirmada"?"confirmed":"pending"}`}>● {trip.status}</span></div><div className="ticket-info"><div className="wide"><small>PASSAGEIRO</small><strong>{trip.passenger.toUpperCase()}</strong></div><div><small>DATA DO EMBARQUE</small><strong>{trip.date}</strong></div><div><small>HORÁRIO</small><strong>{trip.time}</strong></div><div><small>ORIGEM</small><strong>{from}</strong></div><div><small>DESTINO</small><strong>{to}</strong></div><div><small>COMPANHIA</small><strong>{trip.airline}</strong></div><div><small>RESERVA</small><strong>{trip.locator}</strong></div></div><div className="ticket-note"><strong>Antes de viajar</strong><p>Confira os documentos necessários e consulte a companhia aérea para informações atualizadas sobre check-in, bagagem e portão de embarque.</p></div><div className="barcode" aria-hidden="true">|||| || | |||| | | || ||| | |||| || |</div></div><button className="primary-button submit screen-only" onClick={() => window.print()}>⇩ Baixar bilhete em PDF</button><button className="secondary-button full-button screen-only" onClick={()=>window.open(reservationUrl(trip),"_blank")}>↗ Consultar no site da companhia</button><button className="screen-only" onClick={remove} disabled={deleting} style={{width:"100%",height:40,marginTop:14,border:0,background:"transparent",color:"#c5414c",fontSize:10,fontWeight:700}}>{deleting?"Excluindo...":"Excluir viagem"}</button></div>;
}
